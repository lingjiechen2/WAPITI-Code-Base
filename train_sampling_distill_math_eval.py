...
login(token = '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')
...
def main():
    ...
    text_column_question = "question" if "question" in column_names else column_names[0]
    text_column_answer = "answer" if "answer" in column_names else column_names[1]
    ...
    def tokenize_math_function(examples):
    ...
    def tokenize_function(examples):
        ...
        with CaptureLogger(tok_logger) as cl:
            tokenized_question = tokenizer(examples[text_column_question], truncation=True, padding="max_length", max_length=data_args.block_size)
            tokenized_answer = tokenizer(examples[text_column_answer], truncation=True, padding="max_length", max_length=data_args.block_size)

        # Handle any tokenization warnings
        ...
        return {
            ...
            "attention_mask_question": tokenized_question["attention_mask"],
            "input_ids_answer": tokenized_answer["input_ids"],
            "attention_mask_answer": tokenized_answer["attention_mask"],
        }
    ...
    def tokenize_function_group_texts(examples):
        # Tokenize both the question and the answer separately
        with CaptureLogger(tok_logger) as cl:
            tokenized_question = tokenizer(examples[text_column_question])
            tokenized_answer = tokenizer(examples[text_column_answer])
    ...
    def group_texts(examples):
        ...
        result["labels"] = result["input_ids_question"].copy()
        return result
    
    def no_group_texts(examples):
        result = examples
        result['input_ids'] = result['input_ids_question'].copy()
        result['attention_mask'] = result['attention_mask_question'].copy()
        result["labels"] = result["input_ids_answer"].copy()
        return result
    ...
    removed_column_names = ['input_ids_question', 'attention_mask_question', 'input_ids_answer', 'attention_mask_answer']
    ...
    # print(lm_datasets)
    # assert list(lm_datasets['train'].features) == ['attention_mask', 'input_ids', 'labels'], f"Features should be ['attention_mask', 'input_ids', 'labels'], but got {list(lm_datasets['train'].features)}"
    ...
    if training_args.do_eval:
        ...
        tokenized_math_datasets = math_dataset.map(
                tokenize_math_function,
                batched=False,
                num_proc=data_args.preprocessing_num_workers,
                remove_columns=column_names,
                load_from_cache_file=not data_args.overwrite_cache,
        ...
        eval_dataset = tokenized_math_datasets['test']
        ...
        def compute_metrics(eval_preds):
            ...
            generated_answer = tokenizer.decode(preds, skip_special_tokens=True)

            return labels.strip() == generated_answer.strip()
