...
model = HookedTransformer.from_pretrained('Llama-2-7b', device="cpu", hf_model=vanilla_model)
model.to('cuda')
model.eval()
...
for watermark_name, watermark_config in watermark_config.items():
    ...
    torch.save(watermark_outputs_dict, final_result_folder + "tl_watermark_residuals.pt")
    torch.save(watermark_logits_dict, final_result_folder + "tl_watermark_logits.pt")
