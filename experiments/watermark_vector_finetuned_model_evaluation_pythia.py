...
''' Starting Scipt can be: python watermark_vector_finetuned_model_evaluation.py --tested_model_name georgesung/llama2_7b_chat_uncensored --watermark_type kgw

Finetuned LLM choices: 
...
parser = argparse.ArgumentParser(description="Process some arguments for watermark detection.")
parser.add_argument('--tested_model_name', type=str, required=True,
                    help="Path to the language model that will be tested.")
parser.add_argument('--watermark_type', type=str, choices=['kgw', 'aar'], required=True,
                    help="Type of watermark. Must be 'kgw' or 'aar'.")
parser.add_argument('--save_file_name', type=str, default=None,
                    help="Path to save the watermark vector.")
args = parser.parse_args()

tested_model_name = args.tested_model_name
watermark_type = args.watermark_type
save_path = f'/remote-home1/miintern1/watermark-learnability/data/c4/{watermark_type}_watermark_vector_{args.save_file_name}.json'
print(f"Start processing {watermark_type} watermark for model {tested_model_name}")
print(f"Save path: {save_path}")

logging.basicConfig(
    filename=f'/remote-home1/miintern1/watermark-learnability/logs/{watermark_type}_watermark_vector_{args.save_file_name}.txt',  # Specify the log file name
...
num_samples = 512
batch_size = 128
# save_path = f'/remote-home/miintern1/watermark-learnability/data/c4/{watermark_type}_watermark_vector_{tested_model_name}.json'
...
if watermark_type == "kgw":
    ...
    # }
    watermark_configs = {
        "cygu/pythia-1.4b-sampling-watermark-distill-kgw-k0-gamma0.25-delta1":{"type": "kgw", "k": 0, "gamma": 0.25, "delta": 1.0, "seeding_scheme": "simple_0", "kgw_device": "cuda"},
        "cygu/pythia-1.4b-sampling-watermark-distill-kgw-k0-gamma0.25-delta2":{"type": "kgw", "k": 0, "gamma": 0.25, "delta": 2.0, "seeding_scheme": "simple_0", "kgw_device": "cuda"},
        "cygu/pythia-1.4b-sampling-watermark-distill-kgw-k1-gamma0.25-delta1":{"type": "kgw", "k": 1, "gamma": 0.25, "delta": 1.0, "seeding_scheme": "simple_1", "kgw_device": "cuda"},
        "cygu/pythia-1.4b-sampling-watermark-distill-kgw-k1-gamma0.25-delta2":{"type": "kgw", "k": 1, "gamma": 0.25, "delta": 2.0, "seeding_scheme": "simple_1", "kgw_device": "cuda"},
        "cygu/pythia-1.4b-sampling-watermark-distill-kgw-k2-gamma0.25-delta2":{"type": "kgw", "k": 2, "gamma": 0.25, "delta": 2.0, "seeding_scheme": "simple_2", "kgw_device": "cuda"},
    }
    ...
    raise ValueError("Invalid watermark type. Must be 'kgw' or 'aar'.")

# vanilla_model_name = "meta-llama/Llama-2-7b-hf"
vanilla_model_name = 'EleutherAI/pythia-1.4b'
ppl_model_name = 'meta-llama/Llama-2-7b-hf'
...
for watermark_name, watermark_config in watermark_configs.items():
    ...
    elif ([float(value) for value in all_model_dict[watermark_name].keys() if is_float(value)] == coefficient_list).all():
    ...
    for i,coefficient in enumerate(coefficient_list):
        ...
        for batch in tqdm(prompts):
            ...
            del vanilla_output, watermarked_output, vanilla_text, watermarked_text
            torch.cuda.empty_cache()
