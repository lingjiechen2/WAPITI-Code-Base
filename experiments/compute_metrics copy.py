...
def compute_p_values(samples_dict, tokenizer, kgw_device, truncate=False, num_tokens=200):
    ...
    for model_name, sd in tqdm(samples_dict.items()):
        ...
        for coefficient in samples_dict[model_name]:
            samples = samples_dict[model_name][coefficient]["watermarked_output"]
            coefficient_scores = []
            ...
            for s in tqdm(samples):
                ...
                if watermark_type == WatermarkType.KGW:
                    score = score["p_value"]
                coefficient_scores.append(score)
            sd[coefficient]["p_values"] = coefficient_scores
            sd[coefficient]["median_p_value"] = np.median(coefficient_scores)
            print(f"Model name: {model_name}\nMedian p-value: {np.median(coefficient_scores)}")
        del detector
...
def compute_repetition(samples_dict, tokenizer):
    ...
    for model_name, all_sd in tqdm(samples_dict.items()):
        for coefficient, sd in all_sd.items(): 
