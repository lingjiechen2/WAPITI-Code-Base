...
class KGWWatermark:
    ...
    def watermark_logits(self,
        ...
        print(logits[..., self.k - 1:, :mask.shape[-1]][true_indices[0]])
    ...
    #     logits: torch.FloatTensor,  # (batch, seq_len, vocab_size)
    # ) -> torch.FloatTensor:
    #     print('Using Flattening Method')
    #     """Returns watermarked logits to be used as distillation target."""
    #     hashes = torch.sum(input_ids.unfold(-1, self.k, 1), dim=-1)  # (batch, seq_len - k + 1)
    #     mask = self.greenlist_masks[hashes]  # (batch, seq_len - k + 1, vocab_size)
        
    #     # Flatten the mask and logits for modification
    #     flat_mask = mask.view(-1)
    #     print(flat_mask.shape)
    #     flat_logits = logits[..., self.k - 1:, :mask.shape[-1]].view(-1, mask.shape[-1])

    #     # Apply the delta where the mask is True
    #     flat_logits[flat_mask] += self.delta

    #     # Reshape back to the original logits shape
    #     logits[..., self.k - 1:, :mask.shape[-1]] = flat_logits.view_as(logits[..., self.k - 1:, :mask.shape[-1]])
